...
def init_db():
    ...
    IMPORTANT: TIME columns in MySQL may be returned by PyMySQL as timedelta.
    Availability layer will normalize those to datetime.time.
    """
    ddl = """
...
def get_business_by_name(name: str) -> Optional[Dict[str, Any]]:
    with db_cursor() as cur:
        cur.execute("SELECT * FROM businesses WHERE name=%s LIMIT 1", (name,))
        return cur.fetchone()


def get_or_create_business(name: str, *, slug: str = "barber_demo", timezone: str = "Asia/Beirut") -> int:
    row = get_business_by_name(name)
    if row:
        return int(row["id"])
...
def log_call_start(session_id: str, business_id: int, customer_id: Optional[int] = None) -> int:
    with db_cursor() as cur:
        cur.execute(
            "INSERT INTO calls (business_id, session_id, customer_id) VALUES (%s,%s,%s)",
...
def log_message(
    ...
    *,
    ...
    # ✅ FIX: MySQL driver won’t accept dict directly → serialize to JSON string
    entities_json = None
    ...
    with db_cursor() as cur:
        cur.execute(
            """
            INSERT INTO messages (call_id, role, text, intent, confidence, entities_json)
            VALUES (%s,%s,%s,%s,%s,%s)
            """,
