...
import re
from src.config import DIALOG_MODEL
...
- Dates without explicit years use the current year (2024)
...
- "reschedule_appointment": Reschedule an appointment (params: appointment_id OR customer_phone, new_date, new_time)
  IMPORTANT: Use the actual appointment_id from get_customer_appointments result, NOT a placeholder like "original_appointment_id"
  After rescheduling, you may need to book a new appointment with book_appointment using the new date/time
...
- When you have all required info (date, time, service, customer name/phone), use book_appointment action IMMEDIATELY
